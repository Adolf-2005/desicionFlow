        ...
        <v-form ref="form" v-model="valido">
          <template v-for="field in fields">
            <v-col v-if="(field.type === 'text' || field.type === 'password') && (field.row === false) && !field.edit"
              ...
              <v-text-field :data-test="field.title" v-model="record[field.key]" :label="field.title" variant="outlined" density="compact"
                :rules="[rules.required, rules.empty]" :type="field.type" autocomplete="off" :readonly="field.readonly">
              </v-text-field>
            </v-col>

            <v-col v-if="field.type === 'textarea'">
              <v-textarea :data-test="field.title" v-model="record[field.key]" :label="field.title" variant="outlined" density="compact"
            ...
            <v-row class="px-2" v-if="(field.type === 'text' || field.type === 'password') && field.row === true">
              ...
              <v-col cols="12" sm="6">
                <v-text-field :data-test="field.subTitle" v-model="record[field.subKey]" :label="field.subTitle" variant="outlined" density="compact"
            ...
            <v-row class="pa-2" v-if="(field.type === 'select') && field.row === true">
              <v-col cols="5" sm="3">
                <v-select :data-test="field.title" v-model="record[field.key]" :label="field.title" variant="outlined" density="compact"
